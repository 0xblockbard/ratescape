...
@section('content_admin')
    ...
    <div class="w-full pt-2 pb-16 h-full min-h-screen ">
        ...
        <div class="flex items-center max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            <div class="flex mr-3">
                <a href="{{ route('admin_pages_overview') }}" class="hover:text-neongreen-500">
            ...
            <h1 class="flex items-center text-2xl flex-1 font-semibold text-gray-900">
                <span class="w-24 text-base sm:w-auto sm:text-xl">Edit Page:</span>
                <span class="text-gray-800 text-base pl-0 sm:pl-3 sm:text-xl">{{ $page->title }}</span>
            </h1>
            ...
            <div class="flex justify-end">
                ...
                <button type="submit" class="button ml-3" form="page_form">
                    Save Changes
                </button>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            ...
            {!! Form::model($page, ['route' => ['admin_pages_update', $page->uuid], 'files' => 'true', 'method' => 'PUT', 'id' => 'page_form'])  !!}
                ...
                <div class="page_main_content {{ session('toggleSidebar') == true ? 'lg:col-span-12' : 'lg:col-span-9' }} mt-3 sm:mt-2 space-y-6 sm:space-y-5">
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <input type="text" placeholder="Title of Page" value="{{ $page->title }}" name="title" id="title" autocomplete="title" class="block w-full shadow-sm focus:ring-neongreen-300 focus:border-neongreen-500 sm:text-lg font-semibold border-neongreen-300 rounded-md bg-cream-200">
                        </div>
                    </div>

                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <input type="text" placeholder="Tagline" value="{{ $page->tagline }}" name="tagline" id="tagline" autocomplete="tagline" class="block italic w-full shadow-sm focus:ring-neongreen-300 focus:border-neongreen-500 sm:text-sm border-neongreen-300 rounded-md bg-cream-200">
                        </div>
                    </div>
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <div class="w-full flex rounded-md shadow-sm">
                                <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-neongreen-300bg-gray-50 text-gray-500 sm:text-sm">
                                {{ config('app.url').'/' }}
                                </span>
                                <input type="text" placeholder="custom-slug" @if(old('custom_slug_toggle')) @elseif($page->custom_slug !== 1) disabled @endif  value="{{ $page->slug }}" name="custom_slug" id="custom_slug" autocomplete="custom_slug" class="custom_slug flex-1 italic block w-full @if(old('custom_slug_toggle')) @elseif($page->custom_slug !== 1) bg-gray-200 text-gray-600 cursor-not-allowed @endif focus:ring-neongreen-300 focus:border-neongreen-500 min-w-0 rounded-none rounded-r-md sm:text-sm border-neongreen-200">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <textarea id="tinymce" name="content" rows="20" class="shadow-sm block w-full focus:ring-neongreen-300 focus:border-neongreen-500 sm:text-sm border-neongreen-300rounded-md bg-cream-200">{!!  $page->content  !!}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <input type="text" placeholder="Meta Title" value="{{ $page->meta_title }}" name="meta_title" id="meta_title" autocomplete="meta_title" class="block w-full shadow-sm focus:ring-neongreen-300 focus:border-neongreen-500 sm:text-sm border-neongreen-300 rounded-md bg-cream-200">
                        </div>
                    </div>

                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <input type="text" placeholder="Meta Description" value="{{ $page->meta_description }}" name="meta_description" id="meta_description" autocomplete="meta_description" class="block w-full shadow-sm focus:ring-neongreen-300 focus:border-neongreen-500 sm:text-sm border-neongreen-300 rounded-md bg-cream-200">
                        </div>
                    </div>
                ...
                <div class="page_sidebar {{ session('toggleSidebar') == true ? 'hidden' : '' }} col-span-3 lg:col-span-3 lg:pl-6 lg:pr-6 lg:ml-6 lg:border-l lg:border-neongreen-300 rounded mt-3 max-w-3xl sm:mt-3 space-y-6 sm:space-y-5">
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        ...
                        <div class="mt-1 sm:mt-0 sm:col-span-7">
                            <select id="status" name="status" autocomplete="status" class="block focus:ring-neongreen-300 focus:border-neongreen-500 w-full shadow-sm sm:text-sm border-neongreen-300 rounded-md">
                    ...
                    <div class="pt-4 flex items-center justify-between sm:border-t sm:border-neongreen-200" x-data="{ on: {{ $page->custom_slug == 1 ? 'true' : 'false'  }}  }">
                        ...
                        <div class="relative inline-block w-10 mr-2 align-middle select-none">
                            <input type="checkbox" name="custom_slug_toggle" id="custom_slug_toggle"
                                   x-on:click='on = !on' :class="{'right-4': !on, '-right-1': on}" x-bind:checked="{{ $page->custom_slug == 1 ? 'true' : 'false'  }} "
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-neongreen-300 sm:pt-3">
                        <label for="status" class=" sm:col-span-3 block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">
                            Featured Image
                        </label>
                        <div class="mt-1 sm:mt-0 sm:col-span-3">

                            <div class="featured_image {{ $page->featured_image !== NULL ? '' : 'hidden' }}">
                                
                                <div class="flex relative w-full">
                                    <img class="current-image {{ $page->featured_image == NULL ? 'no-image' : '' }} object-cover rounded-md" src="{{ $page->featured_image }}" >
                                ...
                                <div class="flex text-sm text-gray-600 justify-left pt-3">
                                    <label for="featured_image_upload" class="relative cursor-pointer py-1 px-2 mb-2 rounded-md font-medium text-neongreen-600 hover:text-neongreen-500">
                                        ...
                                        <input id="featured_image_update" name="featured_image_update" type="file" uuid="{{ $page->uuid }}" route="{{ route('admin_upload_featured_image') }}" class="featured_image_update sr-only">
                                    </label>
                                </div>
                            </div>

                            <div class="featured_image_upload_container {{ $page->featured_image !== NULL ? 'hidden' : '' }} flex justify-center px-6 pt-5 pb-6 border-2 border-neongreen-300 border-dashed rounded-md">
        ...
        <div class="ml-8 pt-0 mt-12 border-t border-neongreen-200">
