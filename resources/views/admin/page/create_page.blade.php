    ...
    <div class="w-full pt-2 pb-16 h-full min-h-screen ">
        ...
        <div class="flex items-center max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            ...
            <h1 class="flex items-center text-2xl flex-1 font-semibold text-gray-900">
                <span class="w-24 text-base sm:w-auto sm:text-xl">New Page:</span>
                {{--                <span class="text-gray-600 text-base pl-0 sm:pl-3 sm:text-xl">{{ $page->title }}</span>--}}
            </h1>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            ...
            {!! Form::open(['route' => ['admin_pages_store'], 'files' => 'true', 'id' => 'page_form'])  !!}
            ...
            <div class="md:grid md:grid-cols-12 mt-4">
                ...
                <div class="page_main_content {{ session('toggleSidebar') == true ? 'lg:col-span-12' : 'lg:col-span-9' }} mt-3 sm:mt-2 space-y-6 sm:space-y-5">
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <input type="text" value="{{ old('title') }}" placeholder="Title of Page" name="title" id="title" autocomplete="title" class="block sm:text-lg font-semibold w-full shadow-sm focus:ring-neongreen-300 focus:border-neongreen-300 border-neongreen-300 rounded-md bg-cream-200">
                        </div>
                    </div>

                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <textarea name="tagline" placeholder="Tagline" rows="2" style="resize:none" class="shadow-sm block w-full focus:ring-neongreen-300 focus:border-neongreen-300 sm:text-sm border-neongreen-300 rounded-md bg-cream-200">{!!  old('tagline')  !!}</textarea>
                        </div>
                    </div>
                        ...
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <div class="w-full flex rounded-md shadow-sm">
                              <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-neongreen-300 bg-gray-50 text-gray-500 sm:text-sm">
                                ...
                                <input type="text" placeholder="custom-slug" value="{{ old('custom_slug') }}" @if(old('custom_slug_toggle')) @else disabled @endif name="custom_slug"  id="custom_slug" autocomplete="custom_slug" class="custom_slug flex-1 block w-full @if(old('custom_slug_toggle')) @else bg-gray-200 text-gray-600 cursor-not-allowed @endif italic focus:ring-neongreen-300 focus:border-neongreen-300 min-w-0 rounded-none rounded-r-md sm:text-sm border-neongreen-300">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <textarea id="tinymce" name="content" rows="20" class="shadow-sm block w-full focus:ring-neongreen-300 focus:border-neongreen-300 sm:text-sm border-neongreen-300 rounded-md bg-cream-200">{!!  old('content')  !!}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <input type="text" placeholder="Meta Title" value="{{ old('meta_title') }}" name="meta_title" id="meta_title" autocomplete="meta_title" class="block w-full shadow-sm focus:ring-neongreen-300 focus:border-neongreen-300 sm:text-sm border-neongreen-300 rounded-md bg-cream-200">
                        </div>
                    </div>

                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field mt-1 sm:mt-0 sm:col-span-12">
                            <input type="text" placeholder="Meta Description" value="{{ old('meta_description') }}" name="meta_description" id="meta_description" autocomplete="meta_description" class="block w-full shadow-sm focus:ring-neongreen-300 focus:border-neongreen-300 sm:text-sm border-neongreen-300 rounded-md bg-cream-200">
                        </div>
                    </div>
                ...
                <div class="page_sidebar {{ session('toggleSidebar') == true ? 'hidden' : '' }} col-span-3 lg:col-span-3 lg:pl-6 lg:pr-6 lg:ml-6 lg:border-l lg:border-neongreen-300 rounded mt-3 max-w-3xl sm:mt-3 space-y-6 sm:space-y-5">
                    ...
                    <div class="pt-4 flex items-center justify-between sm:border-t sm:border-neongreen-200" x-data="{ on: {{ old('custom_slug') == 1 ? 'true' : 'false'  }}  }">
                        ...
                        <div class="relative inline-block w-10 mr-2 align-middle select-none">
                            <input type="checkbox" name="custom_slug_toggle" id="custom_slug_toggle"
                                   x-on:click='on = !on' :class="{'right-4': !on, '-right-1': on}" x-bind:checked="{{ old('custom_slug') == 1 ? 'true' : 'false'  }} "
                                   class="custom_slug_toggle checked:bg-white focus:bg-white bg-white outline-none right-4 checked:-right-1 duration-200 ease-in absolute block w-5 h-5 top-0.5 rounded-full hover:bg-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neongreen-500 appearance-none cursor-pointer border-neongreen-300border-2"/>
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-neongreen-300 sm:pt-3">
                        ...
                        <div class="mt-1 sm:mt-0 sm:col-span-3">
                            ...
                            <div class="featured_image_upload_container {{ old('featured_image') !== NULL ? 'hidden' : '' }} flex justify-center px-6 pt-5 pb-6 border-2 border-neongreen-300 border-dashed rounded-md">
