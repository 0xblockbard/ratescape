    ...
    <div class="w-full pt-2 pb-16 h-full min-h-screen ">
        ...
        <div class="flex items-center max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            ...
            <h1 class="flex items-center text-2xl flex-1 font-semibold text-gray-900">
                <span class="w-24 text-base sm:w-auto sm:text-xl">New Post:</span>
            </h1>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            ...
            {!! Form::open(['route' => ['admin_blogposts_store'], 'files' => 'true', 'id' => 'post_form'])  !!}
            ...
            <div class="md:grid md:grid-cols-12 mt-4">
                ...
                <div class="page_main_content {{ session('toggleSidebar') == true ? 'lg:col-span-12' : 'lg:col-span-9' }} mt-3 sm:mt-2 space-y-6 sm:space-y-5">
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field sm:col-span-12 mt-1 sm:mt-0">
                            <input type="text" placeholder="Title of Blogpost" value="{{ old('title') }}" name="title" id="title" autocomplete="title" class="block sm:text-lg font-semibold w-full shadow-sm focus:ring-neongreen-300 focus:border-neongreen-300 border-neongreen-300 rounded-md bg-cream-200">
                        </div>
                    </div>

                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field sm:col-span-12 mt-1 sm:mt-0">
                            <textarea name="tagline" placeholder="Tagline" rows="2" style="resize:none" class="shadow-sm block bg-cream-200 w-full focus:ring-neongreen-300 focus:border-neongreen-300 sm:text-sm border-neongreen-300 rounded-md">{!!  old('tagline')  !!}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field sm:col-span-12 mt-1 sm:mt-0">
                            <div class="w-full flex rounded-md shadow-sm">
                              <span class="inline-flex items-center px-3 rounded-l-md border border-r-0 border-neongreen-300 bg-cream-200 text-gray-500 sm:text-sm">
                                {{ config('app.url').'/' }}
                              </span>
                                <input type="text" placeholder="custom-slug" value="{{ old('custom_slug') }}" @if(old('custom_slug_toggle')) @else disabled @endif name="custom_slug" id="custom_slug" autocomplete="custom_slug" class="custom_slug flex-1 block w-full @if(old('custom_slug_toggle')) @else bg-neongreen-100 text-graphite-500 cursor-not-allowed @endif focus:ring-neongreen-300 focus:border-neongreen-300 min-w-0 rounded-none rounded-r-md sm:text-sm border-neongreen-300">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field sm:col-span-12 mt-1 sm:mt-0">
                            <textarea name="excerpt" placeholder="Excerpt" rows="3" style="resize:none" class="shadow-sm block w-full focus:ring-neongreen-300 focus:border-neongreen-300 sm:text-sm border-neongreen-300 bg-cream-200 rounded-md">{!!  old('excerpt')  !!}</textarea>
                        </div>
                    </div>

                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field sm:col-span-12 mt-1 sm:mt-0">
                            <textarea id="tinymce" placeholder="Content" name="content" rows="20" class="shadow-sm block w-full focus:ring-neongreen-300 focus:border-neongreen-300 sm:text-sm border-neongreen-300 bg-cream-200 rounded-md">{!!  old('content')  !!}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field sm:col-span-12 mt-1 sm:mt-0">
                            <input type="text" placeholder="Meta Title" value="{{ old('meta_title') }}" name="meta_title" id="meta_title" autocomplete="meta_title" class="block w-full shadow-sm focus:ring-neongreen-300 focus:border-neongreen-300 sm:text-sm border-neongreen-300  bg-cream-200 rounded-md">
                        </div>
                    </div>

                    <div class="sm:grid sm:grid-cols-10 sm:gap-4 sm:items-start sm:pt-1">
                        <div class="page_main_field sm:col-span-12 mt-1 sm:mt-0">
                            <input type="text" placeholder="Meta Description" value="{{ old('meta_description') }}" name="meta_description" id="meta_description" autocomplete="meta_description" class="block w-full shadow-sm focus:ring-neongreen-300 focus:border-neongreen-300 sm:text-sm border-neongreen-300 bg-cream-200 rounded-md">
                        </div>
                    </div>
                ...
                <div class="page_sidebar {{ session('toggleSidebar') == true ? 'hidden' : '' }} col-span-3 lg:col-span-3 lg:pl-6 lg:pr-6 lg:ml-6 lg:border-l lg:border-neongreen-300 rounded mt-3 max-w-3xl sm:mt-3 space-y-6 sm:space-y-5">
                    ...
                    <div class="pt-4 flex items-center justify-between sm:border-t sm:border-neongreen-300" x-data="{ on: {{ old('custom_slug_toggle') == "on" ? 'true' : 'false'  }}  }">
                        ...
                        <div class="relative inline-block w-10 mr-2 align-middle select-none">
                            <input type="checkbox" name="custom_slug_toggle" id="custom_slug_toggle"
                                   x-on:click='on = !on' :class="{'right-4 b': !on, '-right-1': on}" x-bind:checked="{{ old('custom_slug_toggle') == "on" ? 'true' : 'false'  }} "
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-neongreen-300 sm:pt-3">
                        ...
                        <div class="mt-1 sm:mt-0 sm:col-span-3">
                            ...
                            <div class="featured_image {{ old('featured_image') !== NULL ? '' : 'hidden' }}">

                                <div class="flex relative w-full">
                                    <img class="current-image no-image object-cover rounded-md" src="" >
                                ...
                                <div class="flex text-sm text-gray-600 justify-left pt-3">
                                    <label for="featured_image_upload" class="relative cursor-pointer py-1 px-2 rounded-md font-medium text-neongreen-600 hover:text-neongreen-500 ">
                                        <span>Change featured image</span>
                                        <input id="featured_image_update" name="featured_image_update" type="file" uuid="" route="{{ route('admin_upload_featured_image') }}" class="featured_image_update sr-only">
                                    </label>
                                </div>
                            </div>

                            <div class="featured_image_upload_container flex justify-center px-6 pt-5 pb-6 border-2 border-neongreen-300 border-dashed rounded-md">
        ...
        <div class="ml-8 pt-0 mt-12 border-t border-neongreen-300">
